<template>
    <div class="container-card" v-if="showCardInfo">
        <h1>Crie sua conta</h1>
        <div class="card">
            <div class="start-card-info">
                <div class="balance">
                    <p>Deposito Inicial</p>
                    <input type="number" v-model="saldoInicial">
                </div>
                <div class="balance">
                    <p>Nome Completo</p>
                    <input type="text" v-model="nomeCartao">
                </div>
            </div>
        </div>
        <div class="button-create">
            <button class="btn" @click="toggleCardInfo" :disabled="saldoInicial < 100" v-if="showCardInfo">
                Criar Conta
            </button>
        </div>
        <p v-if="saldoInicial < 100" class="error-message">O valor de deposito inicial deve ser maior que 100 para criar a conta.</p>
    </div>
    <div class="container-card" v-else>
        <h1>My Card</h1>
        <div class="card">
            <div class="start-card-info">
                <div class="balance">
                    <p>Saldo</p>
                    <h1 :class="{ 'positivo': saldoInicial >= 0, 'negativo': saldoInicial <0 }">R$ {{saldoInicial}}</h1>
                </div>
                <div class="card-info">
                    <div class="card-flag">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" x="0px" y="0px" width="100" height="100" viewBox="0 0 50 50"><path d="M 5 7 C 2.242188 7 0 9.242188 0 12 L 0 38 C 0 40.757813 2.242188 43 5 43 L 45 43 C 47.757813 43 50 40.757813 50 38 L 50 12 C 50 9.242188 47.757813 7 45 7 Z M 29.6875 19.40625 C 31.050781 19.40625 32.46875 19.96875 32.46875 19.96875 L 31.96875 22.40625 C 31.96875 22.40625 30.890625 21.6875 29.9375 21.6875 C 28.46875 21.6875 27.9375 22.167969 27.9375 22.8125 C 27.9375 24.074219 32.03125 24.296875 32.03125 27.125 C 32.03125 29.476563 29.113281 31.09375 27 31.09375 C 24.886719 31.09375 23.78125 30.46875 23.78125 30.46875 L 24.3125 28.09375 C 24.3125 28.09375 25.417969 28.75 27.125 28.75 C 28.828125 28.75 29.0625 28.023438 29.0625 27.71875 C 29.0625 25.914063 25 26.417969 25 22.90625 C 25 20.964844 26.585938 19.40625 29.6875 19.40625 Z M 16.46875 19.625 L 19.625 19.625 L 15.125 30.75 L 11.875 30.75 L 9.5 21.75 C 9.5 21.75 11.996094 23.023438 13.53125 26.46875 C 13.597656 26.886719 13.78125 27.5625 13.78125 27.5625 Z M 20.78125 19.625 L 23.78125 19.625 L 22 30.75 L 19.03125 30.75 Z M 36.8125 19.625 L 39.90625 19.625 L 42.1875 30.75 L 39.5 30.75 L 39.1875 29.15625 L 35.5 29.15625 L 34.90625 30.75 L 31.96875 30.75 Z M 6.25 19.65625 L 10.8125 19.65625 C 11.976563 19.65625 12.40625 20.75 12.40625 20.75 L 13.40625 25.8125 C 12.054688 21.453125 6.25 19.65625 6.25 19.65625 Z M 37.9375 22.84375 L 36.3125 27.03125 L 38.75 27.03125 Z"></path></svg>
                    </div>
                    <div class="card-name">
                        <p>{{nomeCartao}}</p>
                    </div>
                </div> 
            </div>
        </div>
        <div class="movimentacao">
            <input type="number" v-model="valorMovimentacao" :min="0">
        </div>
        <div class="button-create">
            <button class="btn" @click="depositarValor">
                Depositar
            </button>
            <button class="btn" @click="sacarValor">
                Sacar
            </button>
        </div>

    </div>
</template>

<script>
    export default {
        name: 'CardStart',
        data() {
            return {
                showCardInfo: true,
                nomeCartao: '',
                saldoInicial: 0,
                valorMovimentacao: 0,
            }
        },
        methods: {
            toggleCardInfo() {
                this.showCardInfo = !this.showCardInfo;
            },
            sacarValor() {
                if (this.valorMovimentacao > 0) { // Verifica se o valor da movimentação é positivo
                const novoSaldo = this.saldoInicial - this.valorMovimentacao;
                if (novoSaldo >= -400) {
                this.saldoInicial = novoSaldo;
                this.valorMovimentacao = 0; // Limpa o campo de movimentação após o saque bem-sucedido
                } else {
                    alert('Saldo insuficiente para o saque.');
                    }
                } 
            },
            depositarValor() {
                if (this.valorMovimentacao > 0) { // Verifica se o valor da movimentação é positivo
                  this.saldoInicial += this.valorMovimentacao;
                  this.valorMovimentacao = 0; // Limpa o campo de movimentação após o depósito bem-sucedido
                } else {
                  alert('Digite um valor válido para o depósito.');
                }
        }
    }
}
</script>

<style lang="sass">
$color1: #131213
$color2: #2C2C2C
$white: #F5F5F5

//card
$pink: #DD45C5
$bluec: #354668
$cardd: $bluec, $pink, $bluec

@mixin gradient($direction, $cardd) 
  background: -webkit-linear-gradient($direction, $cardd)

.positivo 
  color: green
.negativo 
  color: red

body
    background-color: $color1
    margin: 0
    padding: 0
    box-sizing: border-box
    font-family: Arial, Helvetica, sans-serif

.container-card
    display: flex
    flex-direction: column
    justify-content: center
    align-items: center
    width: 50%
    margin-top: 4rem
    margin-left: 30p
    color: $white
    .card
        @include gradient(left, $cardd)
        width: 400px
        height: 250px
        border-radius: 30px
        .start-card-info
            .balance
                font-size: 40px
                color: $white
                padding-left: 30px
                p
                    font-size: 20px
                    margin: 0
                    margin-top: 2rem
                h1
                    margin: 0
            .card-info
                display: flex
                align-items: flex-end
                padding-left: 30px
                gap: 1.5rem
                margin-top: 4.2rem
                .card-flag
                    svg
                        color: $white
                        width: 50px
                        height: 36px
                .card-name
                    color: $white
    .button-create
        margin-top: 2rem
        .btn
            font-family: Arial
            font-size: 14px
            color: $white
            background-color: $color2
            padding: 10px 30px
            border-radius: 50px
            transition : 1000ms
            transform: translateY(0)
            display: flex
            flex-direction: row
            align-items: center
            cursor: pointer
            &:hover
                transition : 900ms
                padding: 10px 50px
                transform : translateY(-0px)
                background-color: $white
                color: $color1


</style>